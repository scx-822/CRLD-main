...
def main(cfg, resume, opts):
    ...
    train_loader, val_loader, num_data, num_classes = get_dataset(
        cfg)  # {DataLoader:2503}, {DataLoader:391}, 1281167, 1000
    ...
    # distillation
        ...
        if cfg.DATASET.TYPE == "imagenet":
            model_teacher = imagenet_model_dict[cfg.DISTILLER.TEACHER](
                pretrained=True)  # imagenet_model_dict[ResNet32](pretrained=True)
            model_student = imagenet_model_dict[cfg.DISTILLER.STUDENT](pretrained=False)
        elif cfg.DATASET.TYPE.startswith("tinyimagenet200"):
            if cfg.DISTILLER.TEACHER == 'deit':
                ...
                model_teacher.load_state_dict(
                    torch.load('../download_ckpts/tinyimagenet_teachers/deit_base_distilled_384.pth')[
                        'model_state_dict'])
                model_student = tinyimagenet200_model_dict[cfg.DISTILLER.STUDENT][0](num_classes=num_classes)
                ...
                for param in model_teacher.parameters():
                    param.requires_grad = False
                model_teacher.reset_classifier(num_classes=200)
                vit_model_prefix = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                ...
                model_teacher = create_model('swin_large_patch4_window7_384', pretrained=False, drop_path_rate=0.1)
                for param in model_teacher.parameters():
                    param.requires_grad = False
                model_teacher.reset_classifier(num_classes=200)
                swin_model_prefix = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                 "..//download_ckpts/tinyimagenet_teachers/")
                swin_model_path = swin_model_prefix + 'swin_large_384'
                model_teacher.load_state_dict(torch.load(swin_model_path)['model_state_dict'])

...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="cifar100",
        choices=["cifar100", "imagenet", "tinyimagenet200"],
    )
    ...
    if args.dataset == "imagenet":
        ...
        if 'vit_large' in args.ckpt:
            cfg.DATASET.TYPE = "tinyimagenet200_384"
            train_loader, val_loader, num_data, num_classes = get_dataset(cfg)
            ...
            train_loader, val_loader, num_data, num_classes = get_dataset(cfg)
            model, pretrain_model_path = tinyimagenet200_model_dict[args.model]
            model = model(num_classes=num_classes)
            ckpt = pretrain_model_path if args.ckpt == "pretrain" else args.ckpt
            model.load_state_dict(load_checkpoint(ckpt)["model"])
            model = Vanilla(model)

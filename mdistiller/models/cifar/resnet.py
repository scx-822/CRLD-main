...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x) # x: (B,256,8,8)->(B,256,1,1)
        avg = x.reshape(x.size(0), -1) # (B,256)
        out = self.fc(avg) # (B,100) (logits)

        feats = {}
        feats["feats"] = [f0, f1, f2, f3]
        feats["preact_feats"] = [f0, f1_pre, f2_pre, f3_pre]
        feats["pooled_feat"] = avg # (B,256) - comes from f3->avgpool (this is 'f4' in PEFD)
        # logits come from f3->avgpool->fc
        return out, feats
